...
from to_csv.a_to_csv import a_main
from to_csv.a2_to_csv import a2_main
...
async def downloadJson(alb, page, start_date, tag):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, headers=headers, params=params, data=data, cookies=cookies) as resp:
            if resp.status == 200:
                res = await resp.json()
                print('success')
            else:
...
async def collect_data(s_alb, s_start_date, msg=None):
    # try:
    alb_dict = {
        ...
        'A2018030720290279534': 'a',
        'A201908281438457360129101': 'a2'
    }
    ...
    for tag in tags:
        ...
        while LoadMore:
            ...
            #     data = read_json(path)
                ...
                # print('exsist')
                # continue
            data = await downloadJson(alb, page, s_start_date, tag)
            if data['errcode'] != 0:
                LoadMore = False
                break
            # print(data)
